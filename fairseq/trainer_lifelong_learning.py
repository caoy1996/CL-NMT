...
class TrainerLifelongLearning(object):
    ...
    def __init__(self, args, task, model, teacher_model, criterion, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task
        ...
        if args.fp16:
            ...
            self._teacher_model = self._teacher_model.half() if teacher_model is not None else None
        if self.cuda:
            self._criterion = self._criterion.cuda()
            self._model = self._model.cuda()
            self._teacher_model = self._teacher_model.cuda() if teacher_model is not None else None
        ...
        self._wrapped_teacher_model = None
    ...
    def teacher_model(self):
        if self._wrapped_teacher_model is None:
            if self._teacher_model is None:
                self._wrapped_teacher_model = None
            elif self.args.distributed_world_size > 1 and not self.args.use_bmuf:
                self._wrapped_teacher_model = models.DistributedFairseqModel(
                    self.args, self._teacher_model
                )
            else:
                self._wrapped_teacher_model = self._teacher_model
        return self._wrapped_teacher_model
    ...
    def load_checkpoint(
        ...
        teacher_checkpoint_path = self.args.restore_teacher_file
        if teacher_checkpoint_path is not None and PathManager.isfile(teacher_checkpoint_path):
            teacher_state = checkpoint_utils.load_checkpoint_to_cpu(teacher_checkpoint_path)
            # load model parameters
            try:
                self.get_teacher_model().load_state_dict(
                    teacher_state["model"], strict=True, args=self.args
                )
            except Exception:
                raise Exception(
                    "Cannot load teacher model parameters from checkpoint {}; "
                    "please ensure that the architectures match.".format(teacher_checkpoint_path)
                )
    ...
    def train_step(self, samples, print_out_norm=False, dummy_batch=False, raise_oom=False):
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.criterion, self.optimizer, print_out_norm,
                        ignore_grad, self.teacher_model
                    )
